...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!hash_equals($csrf,$token)){
        ...
        if($action==='save_variables'){
            $vars=default_vars();
            // Input comes as vars[--color-primary] etc.
            foreach($vars as $k=>$v){
            ...
            $msg = write_file_safely($customCssPath,$next) ? ($lang['Variables saved'] ?? 'Variables saved') : ($lang['Write failed'] ?? 'Write failed');
        } elseif($action==='save_css'){
            $content=(string)($_POST['css_content'] ?? '');
            if(strlen($content)>0 && preg_match('/[{}]/',$content)){
                $msg = write_file_safely($customCssPath,$content) ? ($lang['CSS saved'] ?? 'CSS saved') : ($lang['Write failed'] ?? 'Write failed');
                ...
                if($size>524288){ $err=$lang['Only CSS files allowed'] ?? 'Only CSS files allowed'; }
                    ...
                    if(!preg_match('/\.css$/i',$filename)){ $err=$lang['Only CSS files allowed'] ?? 'Only CSS files allowed'; }
                        ...
                        if(!preg_match('/[{}]/',$data) || preg_match('/<\?php|<script|javascript:/i',$data)){ 
                            ...
                            $msg = write_file_safely($customCssPath,$next) ? ($lang['Upload saved'] ?? 'Upload saved') : ($lang['Upload write failed'] ?? 'Upload write failed');
                        }
                    }
                }
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <button class="btn btn-primary">📤 <?php echo escape($lang['Upload'] ?? 'Upload'); ?></button>
                            </form>
                        </div>
                    </div>
