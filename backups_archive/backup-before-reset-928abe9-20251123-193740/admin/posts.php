...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if($action === 'bulk' && isset($_POST['post_ids']) && isset($_POST['bulk_action'])) {
        ...
        $count = AdminHelper::bulkUpdate($post_ids, $bulk_action);
        
        $message = "$count Beiträge wurden aktualisiert";
    ...
    if(isset($_POST['post_id']) && isset($_POST['quick_action'])) {
        ...
        switch($quick_action) {
            case 'toggle_sticky':
                // Get current status
                $db = Db::get();
                $stmt = $db->prepare("SELECT sticky FROM posts WHERE id = :id");
                $stmt->execute([':id' => $post_id]);
                $current = $stmt->fetch();
                
                $data['sticky'] = !$current['sticky'];
                $message = $data['sticky'] ? 'Beitrag als Sticky markiert' : 'Sticky entfernt';
                break;
                
            case 'toggle_hidden':
                // Get current status
                $db = Db::get();
                $stmt = $db->prepare("SELECT hidden FROM posts WHERE id = :id");
                $stmt->execute([':id' => $post_id]);
                $current = $stmt->fetch();
                
                $data['hidden'] = !$current['hidden'];
                $message = $data['hidden'] ? 'Beitrag versteckt' : 'Beitrag sichtbar';
                break;
        ...
        if(AdminHelper::updatePost($post_id, $data)) {
            $message_type = 'success';
        }
    }
}
